...
class SideBicepCurlTracker:
    """
    Seguimiento y evaluación biomecánica del ejercicio 'Curl Lateral 1 Brazo' (vista lateral).

    Ángulos monitoreados:
        - elbow_angle  : Hombro–Codo–Muñeca. Controla el contador.
        - sway_angle   : Cadera–Hombro–Codo. Detecta balanceo del codo.
        - trunk_angle  : Hombro–Cadera–VerticalRef. Detecta balanceo lumbar.

    Restricciones de postura (en orden de prioridad):
        1. Balanceo lumbar: trunk_angle > 20°.
        2. Balanceo del codo: sway_angle > 30°.
        3. Muñeca por encima del codo al inicio del movimiento (trampa de impulso):
           en la fase "abajo", la muñeca no debe quedar por encima del hombro,
           lo que indicaría que no se llegó a la extensión completa.
    ...
    def __init__(self) -> None:
        ...
        self.stage: Optional[str] = None
        self.feedback = "Buena postura"
        self.name = "Curl Lateral 1 Brazo"
        self.current_angle = 0.0
        self.alpha = 0.4

        self._prev_elbow: Optional[float] = None
        self._prev_sway: Optional[float] = None
        self._prev_trunk: Optional[float] = None
        # Flag: True si en la fase actual de descenso la muñeca estuvo por encima
        # del hombro antes de alcanzar la extensión completa (trampa de impulso).
        self._incomplete_extension: bool = False

    def update(self, landmarks: List[Any], width: int, height: int, side: str = 'left') -> None:
        """
        Actualiza el estado del tracker con los landmarks del frame actual.
        El lado activo se determina automáticamente por visibilidad.
        ...
        try:
            ...
            elbow_angle = calculate_angle(shoulder, elbow, wrist)
            sway_angle  = calculate_angle(hip, shoulder, elbow)
            trunk_angle = calculate_angle(shoulder, hip, v_ref)

            self._prev_elbow = _ema(elbow_angle, self._prev_elbow, self.alpha)
            self._prev_sway  = _ema(sway_angle,  self._prev_sway,  self.alpha)
            self._prev_trunk = _ema(trunk_angle, self._prev_trunk, self.alpha)

            smoothed_elbow = self._prev_elbow
            smoothed_sway  = self._prev_sway
            smoothed_trunk = self._prev_trunk
            self.current_angle = smoothed_elbow

            # --- Contador y detección de trampa de extensión incompleta ---
            # La detección se hace ANTES de actualizar el stage para que el flag
            # capture el estado correcto de transición.

            # Durante la fase de descenso, si la muñeca sube por encima del hombro
            # antes de que el codo se extienda completamente, es trampa de impulso.
            if self.stage != "abajo" and lm_wrist.y - lm_shoulder.y < self.WRIST_ABOVE_SHOULDER_MARGIN:
                self._incomplete_extension = True

            if smoothed_elbow > self.EXTENSION_THRESHOLD:
                self.stage = "abajo"
                self._incomplete_extension = False  # reset al llegar a extensión real

            if smoothed_elbow < self.FLEXION_THRESHOLD and self.stage == "abajo":
                self.reps += 1
                self.stage = "arriba"
